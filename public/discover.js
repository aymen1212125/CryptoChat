...
function chatTile(chat) {
  ...
  button.className = 'chat-tile';
  button.dataset.chat = chat.peer;
  ...
  strong.textContent = `@${chat.peer}`;
  const meta = document.createElement('small');
  meta.textContent = `${chat.messageCount} msgs`;
...
async function loadChats() {
  ...
  chats.forEach((chat) => list.appendChild(chatTile(chat)));
}
